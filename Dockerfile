...
ENV DOCKER_PROJECT_LOCATION=/usr/local/webots-project

# Copy all the benchmark files into default PROJECT_PATH from Docker container
ARG PROJECT_PATH=$DOCKER_PROJECT_LOCATION
...
CMD xvfb-run -e /dev/stdout -a webots --stdout --stderr --batch --mode=fast --no-rendering $DOCKER_PROJECT_LOCATION/worlds/square_path.wbt
